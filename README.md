...
Вывод правил
```
# Список всех правил (таблица filter)
...
```
Открытие порта(-p — протокол (tcp/udp). --dport — порт назначения.)
```
...
```
# Запретить весь входящий трафик, разрешить исходящий
...
📌 Важно: Правила применяются последовательно — первое совпадение определяет судьбу пакета. Все изменения сбрасываются после перезагрузки, если не сохранены! Для сложных сценариев используйте iptables-restore и скрипты.
