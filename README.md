...
[Скрипт](), автоматизирующий настройку базовой безопасности сервера: обновление системы, настройка SSH (отключение парольной аутентификации, изменение порта), установка и настройка firewall (ufw или firewalld).<br>

Скрипт проверяет, был ли передан путь к лог-файлу. Если нет, выводится сообщение об использовании.<br>
`chmod +x security-setup.sh` Сделайте его исполняемым, `sudo ./security-setup.sh` Запустить скрипт, указав путь к лог-файлу<br>
